...
@section('content')
    <div class="row">
        <div class="pull-right" style="margin-right: 100px;">
            <a href="{{ route('billing.printPreview', ['bill_id' => $bill->id ]) }}" class="btnPrint btn btn-primary">Print preview</a>
        </div>
                ...
                <div class="panel-body">
                    ...
                    <div class="col-md-10 col-md-offset-1">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tr>
                                <td colspan="5">ເລກທີ: {{ $bill->id }}<br />ເຖິງ:  {{ $bill->customer_name }}<br />
                                    ເບີໂທລະສັບ: {{ $bill->mobile1 }}
                                    @if(null !== $bill->mobile2 and $bill->mobile2 != '')
                                        , {{ $bill->mobile2 }}
                                    @endif
                                </td>
                                <td colspan="2">ວັນທີ {{ \Carbon\Carbon::parse($bill->created_at)->format('d/m/Y') }}</td>
                            </tr>
                            ...
                            @foreach($sales as $item)
                                @if(in_array($item->product_id, $products))
                                    <tr>
                                        ...
                                        <td>{{ $item->amount }}</td>
                                        <td>{{ $item->unit_eng }}</td>
                                        <td>{{ number_format($item->order_price / $rate) }}</td>
                                        <td>{{ number_format(($item->order_price / $rate) * $item->amount) }}</td>
                                    </tr>
                                @else
                                    <tr>
                                        <td>{{ $no }}</td>
                                ...
                                <?php
                                $no++;
                                $total += (($item->order_price / $rate) * $item->amount);
                                $total_kip += ($item->order_price * $item->amount);
                                ?>
                            @endforeach
                            <!-- include tax 10% -->
                            @if($bill->tax == 1)
                                ...
                                <tr>
                                    ...
                                    <td colspan="4">ຍອດຄ້າງຊຳລະ: {{ number_format((($total) + ($total * 10)/100) - $bill->deposit) }}</td>
                                    <td></td>
                                </tr>
                            @else <!-- exclude tax 10% -->
                                <tr>
